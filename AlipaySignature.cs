...
namespace com._91ios.BaiDuMiniProgram
{
    public class AlipaySignature
    {
        ...
        public static bool RSACheckBD(IDictionary<string, string> parameters, string publicKeyPem, string charset, string signType, bool keyFromFile)
        {
            string sign = parameters["rsaSign"];

            parameters.Remove("rsaSign");
            string signContent = GetSignContent(parameters);
        ...
        public static string RSAEncryptByKeystr(string content, string sPublicKeyPEM, string charset)
        {
            try
            {
                RSACryptoServiceProvider rsa = new RSACryptoServiceProvider();
